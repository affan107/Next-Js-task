...
export default function BillingPage() {
  ...
  return (
    <SettingsLayout title="Billing" icon={CreditCard}>
      <div className="flex flex-col gap-3 max-w-3xl">
        <div>
          <h2 className="text-base font-bold text-[#4A24AB]">
        ...
        <div className="flex gap-8 mt-2">
            ...
            {PLANS.map((plan) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isActive && (
                      <div className="w-2 h-2 rounded-full bg-[#4A24AB]" />
                    )}
                  </div>
          ...
          <div className="flex-1 border-l border-slate-100 pl-8">
            <div className="flex items-baseline gap-1.5 mb-1">
              <span className="text-sm font-bold text-[#4A24AB]">
                {active.summaryTitle}
              </span>
              <span className="text-xs text-slate-400">/</span>
              <span className="text-xs text-slate-500">
                {active.summaryBilling}
              </span>
            </div>
            <p className="text-xs text-slate-500 mb-4">
              {active.summaryTagline}
            </p>
            ...
            {/* Features */}
            <p className="text-xs font-bold text-slate-700 mt-4 mb-2">
              Features
            </p>
            <div className="flex flex-col gap-1.5">
              {active.features.map((f) => (
                <div key={f} className="flex items-center gap-2">
                  <CheckCircle2
                    size={13}
                    className="text-[#15813D] shrink-0"
                    strokeWidth={2}
                  />
            ...
            {/* Divider + upgrade button for non-free plans */}
